...
func (t *TwoStepAuthController) Enable() {
	...
	m, err := two.Enable()
	if err != nil {
		t.JsonError(twoStepEntryType, fmt.Sprintf("%s", err), StringMap{"enable": "no", "username": username}, true)
		return
	}
...
func (t *TwoStepAuthController) Disable() {
	...
	err := two.Disable()
	if err != nil {
		t.JsonError(twoStepEntryType, fmt.Sprintf("%s", err), StringMap{"disable": "no", "username": username}, true)
		return
	}
...
func (t *TwoStepAuthController) Auth() {
	...
	t.JsonError(twoStepEntryType, fmt.Sprintf("%s", err), StringMap{"username": username, "issuer": issuer, "auth": isok}, true)
}
